...
public class Lexical {
	...
	public  String keyWord(String line) {
		...
		switch(word[i]) {
		case "Snl_Start" : 
			resultat =   word[i] +  ":   Mot réservé pour Debut du programme";
			break;
		case "Snl_Close" : 
			resultat =   word[i] +  ":  Mot réservé pour Fin du programme";
			break;
		case "Snl_Int" :
			resultat = word[i] + ":   Mot réservé pour Declaration d'un entier \n" +  lex(line);
			break;
		case "Snl_Real" : 
			resultat =word[i] + ":   Mot réservé pour Declaration d'un reel  \n" + lex(line);
			break;
		case "Snl_Put" : 
			resultat =word[i] + ":    Mot réservé pour  affichage \n" + lex(line);
			break;
		case "SnlSt" : 
			resultat =word[i] + ":   Mot réservé pour declaration d'un reel  \n" + lex(line);
			break;
		case "%." : 
			resultat = word[i] + ":    Mot réservé pour fin d'instruction";
			break;
		case "%.." : 
			resultat =  word[i] + ":    Mot réservé pour debut de commentaire";
			break;
		...
		case "Get" : 
			...
			break;
		case "If" : 
			resultat =word[i] + "  :  Mot réservé pour une condition " +  snl_if(line);
			break;
		...
		case "Else" : 
			...
			break;
		case "Start" : 
			resultat =word[i] +  " :    Mot réservé pour debut de bloc";
			break;
		case "Finish" : 
			resultat =word[i] +  " :    Mot réservé pour fin de bloc";
			break;	
			default : 
				resultat =word[i] +  " :   Erreur mot non reconnu" +	lex(line);
				break;
		}
		return resultat;
	}
		...
		// TODO Auto-generated method stub
 		...
 		for( i = i+1	; i<word.length;i++)
 		{
 			...
 			if(word[i].equals("from")) {
 				word[i] = word[i] + "  :   Mot réservé du langage pour affectation ";
 			}
 		...
 		if(t) { 
 			...
 			chaine_cara = chaine_cara + "   :   chaine de caractere non reconue  ";
 		}
 		resultat = resultat + "\n" +  chaine_cara;
 			}
  		...
  		if(t) { 
  			...
  			chaine_cara = chaine_cara + "   :   chaine de caractere non reconnue , erreur  ";
  		}
  		resultat = resultat + "\n" +  chaine_cara;
 			}
 	...
 	public  String snl_if(String line) {
 		...
 		for( i = i+1; i<word.length;i++) {
 			...
 			if(ident(word[i])) {
 				...
 				word[i] = word[i] + "  :  operation arithmétique";
 			}
 			else if (real(word[i])) {
 				word[i] = word[i] + "  :  un nombre réel";
 			}
 				...
 				word[i] = word[i] + "  : Mot réservé pour debut d'une condtion  ";
 				nbr = 2;	
 			}
 			else if(word[i].equals("%") && nbr == 2) {
 				
 				word[i] = word[i] + "  : Mot réservé pour fin d'une condtion   ";
 				 ...
 				 if(word[i].contains("<=")) {
						String[] sub_word = word[i].split("\\s*<=\\s*");
	 					for(int j =0; j< sub_word.length;j++) {
	 						...
	 						word[i] = word[i] +"    " + sub_word[j] + " :  identificateur \n";
	 	 						...
	 	 						word[i] = word[i] +"        " +sub_word[j] + "  :  un nombre réel \n";
	 	 					}
	 						else {
	 							word[i] = word[i] +"    " +sub_word[j] + "  :    Erreur: mot non reconnu \n";
	 						}
 					...
 					word[i] = word[i] + "  <   Operation arithmétique ";
 						...
 						word[i] = word[i] + "  :   Erreur: mot non reconnu";
	...
	public String[] divide(String code)
	{	
